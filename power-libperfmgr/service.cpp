...
int main() {
    ...
    if (android::base::GetBoolProperty(kConfigDebugPathProperty.data(), false)) {
        ...
        LOG(WARNING) << "Allwinner Power HAL AIDL Service is using debug config from: " << config_path;
    }
    config_path.append(
            android::base::GetProperty(kConfigProperty.data(), kConfigDefaultFileName.data()));

    LOG(INFO) << "Allwinner Power HAL AIDL Service with Extension is starting with config: "
              << config_path;
    ...
    LOG(INFO) << "Allwinner Power HAL AIDL Service with Extension is started.";
    ...
    // should not reach
    LOG(ERROR) << "Allwinner Power HAL AIDL Service with Extension just died.";
    return EXIT_FAILURE;
}
