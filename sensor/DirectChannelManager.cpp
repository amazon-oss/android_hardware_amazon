...
DirectChannelManager::DirectChannelManager() {
    ...
    mSensorToChannel.emplace(ID_GYROSCOPE, std::unordered_map<int32_t, DirectChannelTimingInfo>());
    mSensorToChannel.emplace(ID_MAGNETIC, std::unordered_map<int32_t, DirectChannelTimingInfo>());
...
int DirectChannelManager::mergeDirectReportRequest(int handle) {
    ...
    if (maxRateLevel != SENSOR_DIRECT_RATE_STOP) {
        if (rateChg) ret = mSensorManager->batch(mDirectConnection, handle, period, 0);
        if (activateChg) ret = mSensorManager->activate(mDirectConnection, handle, true);
    } else {
        if (activateChg) ret = mSensorManager->activate(mDirectConnection, handle, false);
    }
    return ret;
}
...
int DirectChannelManager::removeDirectChannel(int channel_handle) {
    ...
    if (!activeSensorList.empty()) {
        ...
        ALOGW("Removing channel %d when sensors (%s) are not stopped.", channel_handle,
              ss.str().c_str());
    }
...
int DirectChannelManager::stopAllDirectReportOnChannel(int channel_handle,
                                                       std::vector<int32_t>* activeSensorList) {
    android::Mutex::Autolock autoLock(mDirectChannelLock);
...
int DirectChannelManager::configDirectReport(int sensor_handle, int channel_handle,
                                             int rate_level) {
    ALOGI("DirectChannel handle=%d, channel_handle=%d, rate_level=%d", sensor_handle,
    ...
    if (rate_level == SENSOR_DIRECT_RATE_STOP) {
        ...
        /* must return (sensor_handle + ID_OFFSET) when non rate stop, cts use return value to check
         * databuffer */
