...
int __vitals_log_ext_print(const char *tag, const char *program,
    ...
    ALOGV("[%s]: tag=%s, program=%s, source=%s, key=%s, cv=%f, unit=%s, "
          ...
          __func__, tag, program, source, key, cv, unit, metadata, ext_data,
          type, is_counter);
    return 0;
}

void __vitals_log_ext_print_v2(
    uint32_t log_level, const char *tag, const char *program,
    ...
    vitals_type type, int is_counter, uint32_t thread_id, uint32_t user_id) {
    ...
    ALOGV("[%s]: log_level=%u, tag=%s, program=%s, source=%s, key=%s, cv=%f, "
          ...
          __func__, log_level, tag, program, source, key, cv, unit, metadata,
          event_timestamp, process_id, log_flag != NULL ? *log_flag : 0,
          ext_data != NULL ? ext_data->ext_size[0] : 0, type, is_counter,
          thread_id, user_id);
}
