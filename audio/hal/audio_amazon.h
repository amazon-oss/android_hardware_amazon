...
struct amazon_audio_hw_device {
     ...
     */
    uint32_t (*get_supported_devices)(const struct amazon_audio_hw_device* dev);

    /**
     * check to see if the audio hardware interface has been initialized.
     * returns 0 on success, -ENODEV on failure.
     */
    int (*init_check)(const struct amazon_audio_hw_device* dev);

    /** set the audio volume of a voice call. Range is between 0.0 and 1.0 */
    int (*set_voice_volume)(struct amazon_audio_hw_device* dev, float volume);
     ...
     */
    int (*set_master_volume)(struct amazon_audio_hw_device* dev, float volume);
     ...
     */
    int (*get_master_volume)(struct amazon_audio_hw_device* dev, float* volume);
     ...
     */
    int (*set_mode)(struct amazon_audio_hw_device* dev, audio_mode_t mode);

    /* mic mute */
    int (*set_mic_mute)(struct amazon_audio_hw_device* dev, bool state);
    int (*get_mic_mute)(const struct amazon_audio_hw_device* dev, bool* state);

    /* set/get global audio parameters */
    int (*set_parameters)(struct amazon_audio_hw_device* dev, const char* kv_pairs);

    /*
     * Returns a pointer to a heap allocated string. The caller is responsible
     * for freeing the memory for it using free().
     */
    char* (*get_parameters)(const struct amazon_audio_hw_device* dev, const char* keys);
    ...
    void (*close_input_stream)(struct amazon_audio_hw_device* dev, struct audio_stream_in* stream_in);

    /** This method dumps the state of the audio hardware */
    int (*dump)(const struct amazon_audio_hw_device* dev, int fd);

    /**
     * set the audio mute status for all audio activities.  If any value other
     * than 0 is returned, the software mixer will emulate this capability.
     */
    int (*set_master_mute)(struct amazon_audio_hw_device* dev, bool mute);
     ...
     */
    int (*get_master_mute)(struct amazon_audio_hw_device* dev, bool* mute);
     ...
     */
    int (*get_audio_port)(struct amazon_audio_hw_device* dev, struct audio_port* port);
     ...
     */
    int (*add_device_effect)(struct amazon_audio_hw_device* dev, audio_port_handle_t device,
     ...
     * @return retval operation completion status.
     */
    int (*remove_device_effect)(struct amazon_audio_hw_device* dev, audio_port_handle_t device,
                                effect_handle_t effect);
     ...
     */
    int (*get_audio_port_v7)(struct amazon_audio_hw_device* dev, struct audio_port_v7* port);
     ...
     * @return retval operation completion status.
     */
    int (*set_device_connected_state_v7)(struct amazon_audio_hw_device* dev,
...
struct audio_patch_record {
    ...
    audio_patch_handle_t handle_amazon; // Handle returned by the original HAL
